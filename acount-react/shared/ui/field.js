...
const fieldWrapper = ({ ControlComponent, fieldType }) =>
  class extends React.PureComponent {
    constructor(props) {
      ...
      const { id, label, darkBg, onChange, onBlur, value, ...rest } = props
      ...
      const wrapperClassName = classNames(
        ...
        {
          [`${BASE_CLASS}--type-${type}`]: type,
          [`${BASE_CLASS}--dark-background`]: darkBg,
        }
      )
      ...
      this.state = {
        ...
        labelProps,
        controlProps,
        localValue: isNil(props.defaultValue) ? ensureValue(value) : null,
    ...
    componentDidMount() {
      const { state } = this
      const { controlProps } = state
      const controlValue =
        `defaultValue` in controlProps
          ? controlProps.defaultValue
          : state.localValue
      this.setState(prevState => {
        return { isEmpty: isEmpty(controlValue) }
      })
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
      ...
      if (isOptionsUpdate) {
        update.controlProps = update.controlProps.set(
          `options`,
          nextProps.options
        )
      }
    ...
    render() {
      ...
      const WrapperClassName = [
        ...
        isTouched
          ? `${BASE_CLASS}--is-touched`
          : `${BASE_CLASS}--is-not-touched`,
      ].join(` `)
